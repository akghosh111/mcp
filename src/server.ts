...
server.tool("create-random-user", "Create a random user with fake data",{
    ...
    try {
        ...
        return {
            content: [{
                type: "text", text: `User ${id} created successfully with the fake data`
            }]
        }
    } catch {
        return {
            content: [
                { type: "text", text: "Failed to parse user data" }
            ]
        }
    }
})
...
async function createUser(user: {
    ...
    //experimental nodejs import feature without the filesystem bs
    const users = await import("./data/users.json", {
        with: {type: "json"}
    }).then(m=> m.default)
